...
def openUnstitched(group):
  ...
  for g in group:
    ...
    for gg in group:
  	  ...
  	  if checkKeys2:
  	    ...
  	    elif gg['channel'] == cycleChannel: existsCycle = True
    if not (existsCm and existsNuc and existsChannel): continue
    fileName = g['fileName']
    channel = g['channel']
    checkKeys
    ...
    elif channel == cycleChannel: cycleIpArr.append(opener.openimage(folderPath + fileName))
  nucStack = ImageStack(nucIpArr[0].width, cmIpArr[0].height)
  cmStack = ImageStack(nucIpArr[0].width, cmIpArr[0].height)
  cycleStack = ImageStack(nucIpArr[0].width, cmIpArr[0].height)
  for nucIp, cmIp, cycleIp in nucIpArr, cmIpArr, cycleIpArr:
    (nucStack.addSlice(nucIp.getTitle(), nucIp.getProcessor()), cmStack.addSlice(cmIp.getTitle(), cmIp.getProcessor()), cycleStack.addSlice(cycleIp.getTitle(), cycleIp.getProcessor()))
  ...
  cycleIp = ImagePlus("images{}".format(fileName), cycleIpArr[0].getProcessor())
  (nucIp.setStack(nucStack), cmIp.setStack(cmStack), cycleIp.setStack(cycleStack))
  return nucIp, cmIp, cycleIp
...
def openStitched(group, rowNo, colNo):
  ...
  for g in group:
    ...
    elif g['channel'] == cycleChannel: cyclech = g['fileName']
  if nucch == "" or cellch == "" or cyclech == "":
    print("Match not Found", group)
    return None, None, None
  nucIp = opener.openImage(folderPath + nucch)
  cmIp = opener.openImage(folderPath + cellch)
  cycleIp = opener.openImage(folderPath + cyclech)
  ...
  IJ.run(cmIp, "Montage to Stack...", "columns={} rows={} border=0".format(colNo, rowNo))
  cmIp = WindowManager.getImage("Stack")
  cmIp.setTitle("CmStain")
  IJ.run(cycleIp, "Montage to Stack...", "columns={} rows={} border=0".format(colNo, rowNo))
  cycleIp = WindowManager.getImage("Stack")
  cycleIp.setTitle("CycleStain")
  nucIp.show(), cmIp.show(), cycleIp.show()
  return nucIp, cmIp, cycleIp
...
folderPath, formatString, groupBy, nucChannel, cmChannel, cycleChannel, stitched, analyzeNucStack, brightfield, nucMethod, cmMethod, analyzeCmStack, rowNo, colNo, nucMinSize, cmMinSize =\
"",          "",          "",      "DAPI",      "GFP",    "EDU",            False,    True,            False,      "Otsu",    "Triangle", True,         8,     7,      50,        500
__dict__ = globals()
try:
...
gd.addStringField("Cell Cycle Image Channel Name", cycleChannel, 70)
...
for outerPairs, group in getCombos(reList, groupBy).iteritems():
  print(group)
  nucIp, cmIp, cycleIp = openStitched(group, rowNo, colNo) if stitched else openUnstitched(group)
  if nucIp is None or cmIp is None or cycleIp is None: continue
  nucStack, cmStack, cycleStack = nucIp.getStack(), cmIp.getStack(), cycleIp.getStack()
  nucIpMedian = calcMedian(nucIp)
  IJ.run(nucIp, "Subtract...", "value=" + str(nucIpMedian) + " stack")
  cycleIpMedian = calcMedian(cycleIp)
  IJ.run(cycleIp, "Subtract...", "value=" + str(cycleIpMedian) + " stack")
  ...
  #or zero if not
  IJ.run("Set Measurements...", "median stack decimal=3")
  Analyzer.setRedirectImage(cycleIp)
  IJ.run(nucAnalyzeIp, "Analyze Particles...", "display stack")
  rt = ResultsTable.getResultsTable()
  groupString = '_'.join("%s=%s" % (key,''.join(val)) for (key,val) in zip(groupBy, outerPairs))
  rt.saveAs(outputFolder +r"/cyclenuclei_" + groupString + ".csv")
  IJ.run("Clear Results")
  rt = ResultsTable.getResultsTable()
  #This finds the median brightness of each nucleus in the cell cycle stain channel
